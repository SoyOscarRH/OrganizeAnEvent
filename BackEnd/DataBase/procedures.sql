...
BEGIN
    SELECT DISTINCT Guest.RFC, CONCAT(Guest.Name, ' ', Guest.FirstSurname, ' ', Guest.SecondSurname) AS FullName, Guest.Email, Place.Name as PlaceName, GuestEvent.Seat, GuestEvent.Assistance, GuestEvent.Username, GuestEvent.Comment
    From Guest, GuestEvent, Place, User, UserEvent
    WHERE 
        GuestEvent.EventID  = ThisEventID    AND
        GuestEvent.RFC      = Guest.RFC      AND
