...
CREATE TABLE IF NOT EXISTS `OAE`.`guest_prize` (
  `rfc` VARCHAR(10) NOT NULL,
  `idprize` INT NOT NULL,
  PRIMARY KEY (`rfc`, `idprize`),
  INDEX `gp_prize_idx` (`idprize` ASC),
  CONSTRAINT `gp_guest`
    FOREIGN KEY (`rfc`)
...
CREATE TABLE IF NOT EXISTS `OAE`.`news` (
  ...
  `idprize` INT NOT NULL,
  PRIMARY KEY (`news`, `idprize`),
  INDEX `news_prize_idx` (`idprize` ASC),
  CONSTRAINT `news_prize`
    FOREIGN KEY (`idprize`)
    REFERENCES `OAE`.`prize` (`idprize`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
